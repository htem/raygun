...
class AddLocalShapeDescriptor():
    ...
    def __init__(
        ...
        self.context = None
        # self.skip = False
        # self.mask_out_background = mask_out_background
    ...
    def process(self, batch, request):
        ...
        if self.mask and self.mask in request:
            ...
            if self.mask_out_background:
                channel_mask = (segmentation_array.crop(descriptor_roi).data!=0).astype(np.float32)
            else:
                channel_mask = np.ones_like(segmentation_array.crop(descriptor_roi).data, dtype=np.float32)

            if self.unlabeled:
                unlabeled_array = batch.arrays[self.unlabeled].data
                channel_mask = np.logical_and(channel_mask, unlabeled_array).astype(np.float32)
    ...
    def __get_descriptors(self, segmentation, roi):
        ...
        # segmentation = segmentation.to_ndarray()
    ...
    def add_lsd_blockwise(
        ...
        # segmentation_array = batch.arrays[self.segmentation]
        # segmentation_array = segment_ds[block.read_roi]

        seg_ndarray = np.zeros(block.read_roi.get_shape(), dtype=np.float32)
        seg_array = daisy.Array(seg_ndarray, block.read_roi, segment_ds.voxel_size)

        intersect_roi = block.read_roi.intersect(segment_ds.roi)
        seg_array[intersect_roi] = segment_ds[intersect_roi]

        # get voxel roi of requested descriptors -- this is the only region in
        # which we have to compute the descriptors
        seg_roi = seg_array.roi
        # descriptor_roi = request[self.descriptor].roi
        descriptor_roi = block.write_roi
        voxel_roi_in_seg = (
            seg_roi.intersect(descriptor_roi) -
            seg_roi.get_offset()) / self.voxel_size

        descriptor = self.__get_descriptors(
            seg_array.to_ndarray(),
            voxel_roi_in_seg)

        descriptor *= 255

        out_ds[block.write_roi] = descriptor

        # logger.info("Finished %s" % block)
        return 0
...
if __name__ == "__main__":
    ...
    # context = daisy.Coordinate(add_lsd.context)
    context = daisy.Coordinate((80*3, 80*3, 80*3))
    read_roi = write_roi.grow(context, context)
    read_roi = read_roi.snap_to_grid(
        segment_ds.voxel_size, mode='grow')
    total_roi = segment_ds.roi
    ...
    daisy.run_blockwise(
        ...
        read_roi,
        write_roi,
        process_function=lambda b: add_lsd.add_lsd_blockwise(
            b,
            segment_ds, lsd_ds,
            ),
        num_workers=48,
