...
logger = logging.getLogger('task_grow_segmentation')
...
class GrowSegmentationTask(task_helper.SlurmTask):
    ...
    grow_db_collection = daisy.Parameter(None)
    skipped_db_collection = daisy.Parameter(None)
    ...
    def process_block(self, block):
        ...
        for seed in self.seed_zyxs:
            ...
            if not roi.contains(seed_zyx):
                ...
                raise RuntimeError
                logger.warn("Seed %s not in ROI %s!" % (seed_zyx, roi))
                skipped_seeds.add((seed_label, seed_zyx))
                continue

            logger.info("Seed %s: %s" % (seed_zyx, seed_label))
        ...
        logger.info("Labels to join %s", disjointed_labels)
    ...
    def remove_grow_nodes(self, ids):
        ...
        if self.grow_db_collection is None:
            return

        if len(ids):
            db = self.rag_provider.get_db()[self.grow_db_collection]
            logger.info("Removing seeds %s" % ids)
            db.remove(
    ...
    def log_to_skippped_db(self, ids, zyxs):
        ...
        if self.skipped_db_collection is None:
            return
    ...
    def prepare(self):
        ...
        logger.info("Reading fragments from %s", self.fragments_file)
    ...
    def pre_check(self, block):
        ...
        # skip if any seed lie outside ROI
        # note: script will errors out in the process function if not all of
        # them are in ROI
        if not block.write_roi.contains(
                daisy.Coordinate(self.seed_zyxs[0])):
            logger.info("Skipping %s because seed %s not in ROI", block, self.seed_zyxs[0])
            return True

        if (block.read_roi.intersects(self.segment_ds.roi) !=
                block.read_roi):
    ...
    def requires(self):
        return []
        return [BlockwiseSegmentationTask(
