...
class Compare:
    ...
    def make_pipes(self):
        ...
        for key, name in self.array_dict.items():
            self.voxel_sizes[key] = self.source._Hdf5LikeSource__read_spec(
                key, data_file, name
            ).voxel_size
    ...
    def plot_results(self, results=None, metric_list=None):
        ...
        fig, axs = plt.subplots(
            ...
            len(metric_list),
            sharey=True,
            figsize=(len(metric_list) * 3, len(results.columns) // 2),
        )
    ...
    def plot_norm_results(self, norm_name=None, results=None, metric_list=None):
        ...
        norm_results = (
            results.divide(getattr(results, norm_name), axis="rows").drop(
                drop_list, axis=1
            )
            - 1
        )
    ...
    def patch_compare(self, patch_size=gp.Coordinate((64, 64, 64))):
        pipeline = self.source + self.normalizers + self.random_location

        request = gp.BatchRequest()
        for array in self.array_dict:
            request.add(
                array,
                self.voxel_sizes[array] * patch_size,
                voxel_size=self.voxel_sizes[array],
            )
    ...
    def compare(
        self, patch_size=gp.Coordinate((64, 64, 64))
    ):  # TODO: FIX MISSING REGIONS ROIS
    ...
    def process(self, batch, request):
        ...
        if self.results is None:
            ...
            self.results += pd.DataFrame(
                results_dict
            )  # TODO: DETERMINE BEST WAY TO COMBINE ACROSS SCANS...
        self.iter += 1
