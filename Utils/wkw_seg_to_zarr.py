...
def get_wk_mask(
    ...
    if save_name is not None:
        ...
        def save_chunk(block:daisy.Roi):
            # try:
            destination.__setitem__(block.write_roi, mask_array.__getitem__(block.read_roi))
            #     return 0 # success
            # except:
            #     return 1 # error
        ...
        if success:
            print(f'{target_roi} from {annotation_name} written to {zarr_path}/{save_name}')
            return destination
        else:
...
if __name__ == '__main__':
    ...
    ap.add_argument("annotation_ID", type=str, help='Input wkw annotation ID (e.g. 626fd7b6010000a80079d0d9)')
    ap.add_argument("save_path", type=str, help='Input path to archive wkw annotation zipfile.')
    ap.add_argument("zarr_path", type=str, help='Input path to Zarr volume used for annotation.')
    ap.add_argument("raw_name", type=str, help='Input name of dataset in the Zarr volume that was used for annotation.')
    ap.add_argument("--wk_url", type=str, help='URL of Webknossos instance.', default='http://catmaid2.hms.harvard.edu:9000')
    ap.add_argument("--annotation_url_prefix", type=str, help='URL prefix for the Webknossos annotation.', default='http://catmaid2.hms.harvard.edu:9000/annotations/Explorational/')
    ap.add_argument("--wk_token", type=str, help='Authentification token for Webknossos instance.', default="Q9OpWh1PPwHYfH9BsnoM2Q")
    ap.add_argument("--gt_name_prefix", type=str, help='Prefix for saving annotation layer to Zarr.', default='volumes/')
