...
class LaunchableDaisyTask():
    ...
    def generateSbatchScript(
        ...
        text.append("#SBATCH -t %d:00:00" % cpu_time)

        if gpu is not None:
            text.append("#SBATCH -p gpu_connectomics")
            text.append("#SBATCH --account=connectomics_contrib")
            if gpu == '' or gpu == 'any':
        ...
        if gpu is not None:
            text.append("module load gcc/9.2.0")
            text.append("module load cuda/11.2")
        text.append("")
        text.append(run_cmd)
