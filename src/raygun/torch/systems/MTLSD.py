...
from raygun.io import MTLSDDataPipe
from raygun.torch.models import MTLSDModel
from raygun.torch.losses import WeightedMSELoss_MTLSD
from raygun.torch.optimizers import get_base_optimizer
from raygun.torch.systems import BaseSystem


class MTLSD(BaseSystem):
    def __init__(self, config=None):
        super().__init__(
            default_config="../default_configs/default_MTLSD_conf.json",
            config=config,
        )
    ...
    def setup_model(self):
        self.model = MTLSDModel(**self.model_kwargs)
    ...
    def setup_datapipes(self):
        ...
        self.datapipes = {"main": MTLSDDataPipe(**dp_kwargs)}
...
if __name__ == "__main__":
    system = MTLSD(config="./train_conf.json")
