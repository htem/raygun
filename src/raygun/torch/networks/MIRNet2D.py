...
class MSRB(nn.Module):
    def __init__(self, n_feat, height, width, stride, bias):
        ...
        self.blocks = nn.ModuleList(
            [
                nn.ModuleList([DAU(int(n_feat * stride**i))] * width)
...
class RRG(nn.Module):
    def __init__(self, n_feat, n_MSRB, height, width, stride, bias=False):
        ...
        modules_body = [
            MSRB(n_feat, height, width, stride, bias) for _ in range(n_MSRB)
        ]
...
class MIRNet(nn.Module):
    def __init__(
        ...
        modules_body = [
            RRG(ngf, n_MSRB, height, width, stride, bias) for _ in range(n_RRG)
        ]
        self.body = nn.Sequential(*modules_body)
