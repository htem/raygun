...
class NotACycleLoss(BaseCompetentLoss):
    ...
    def backward_D(self, data_dict, n_loop=5):
        ...
        self.set_requires_grad(
            [self.netD], True
        )  # D does require gradients when optimizing D
    ...
    def backward_G(self, data_dict):
        ...
        if data_dict["A"]["real"].size()[-self.dims :] != cyc_size:
            losses["A", "l1_loss"] = self.l1_loss(
                self.crop(data_dict["A"]["real"], cyc_size), data_dict["A"]["cycled"]
            )
            losses["B", "l1_loss"] = self.l1_loss(
                self.crop(data_dict["B"]["real"], cyc_size), data_dict["B"]["cycled"]
            )
        else:
            losses["A", "l1_loss"] = self.l1_loss(
                data_dict["A"]["real"], data_dict["A"]["cycled"]
            )
            losses["B", "l1_loss"] = self.l1_loss(
                data_dict["B"]["real"], data_dict["B"]["cycled"]
            )
