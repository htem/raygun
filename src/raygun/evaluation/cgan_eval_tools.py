...
def pick_checkpoints(
    meta_log_dir="/n/groups/htem/ESRF_id16a/tomo_ML/ResolutionEnhancement/raygun/CycleGAN/eccv-bic-2022/tensorboard/",
    increment=10000,
    ...
    for folder in folders:
        ...
        while start not in model_logs[model_name]["step"] and p < len(log_paths):
            model_logs[model_name] = parse_events_file(
                log_paths[p], tags_dict[model_type]
            )
            p += 1

        model_logs[model_name]["geo_mean"] = get_geo_mean(
            model_logs[model_name], tags_dict[model_type]
        )
        ...
        model_logs[model_name]["smooth_geo_mean"] = get_geo_mean(
            model_logs[model_name], tags_dict[model_type], smoothing=smoothing
        )
        ...
        if type not in bests.keys() or bests[type]["score"] > this_best_score:
            bests[type] = {
                ...
                "layer_name": get_best_layer(
                    type, "#INSERT SEED#", model_logs[model_name]["best_step"]
                ),
            }
...
def get_best_layer(
    ...
    type_dict={"split": "SplitNoBottle", "link": "LinkNoBottle"},
):
...
# %%
def try_patch(
    cycleGun, side="A", pad=0, mode="eval", side_length=512, real=None, batch=None
):
    if real is None:
        if batch is None:
            batch = cycleGun.test_prediction(
                side.upper(), side_length=side_length, cycle=False
            )
...
def get_real(cycleGun, side, roi=None):
    ds = daisy.open_ds(
        getattr(cycleGun, f"src_{side}"), getattr(cycleGun, f"{side}_name")
    )
    if roi is None:
        roi = get_center_roi(
            ds,
            cycleGun.side_length,
            cycleGun.ndims,
...
cycleGun.load_saved_model(
    base
    + "/resnet_track001/models/cycleGAN_setups_set20220728_resnet_track001_checkpoint_100000"
)
#%%
